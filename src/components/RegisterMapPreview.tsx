 ...
 *
 * Open with `?preview=regmap`. Renders real sensor + RTC cards (same bodies +
...
import { Thermometer, Activity, Cpu, Clock } from 'lucide-react'
      ...
      <div className="relative mx-auto flex max-w-6xl flex-col gap-6 px-6 py-10">
        <header className="space-y-1">
          ...
          <p className="max-w-xl text-sm text-muted-foreground">
            SVD-inspired JSON maps drive a live inspector on every register-file card
            (sensors and the PCF8523 RTC) — collapsed until you open{' '}
...
function RtcPreviewCard({ chip }: { chip: RtcChip }) {
  const hex = chip.address.toString(16).padStart(2, '0')
  return (
    <section className="w-[22rem] overflow-hidden rounded-lg border border-border bg-card shadow-xl">
      <header className="flex items-center gap-2 border-b border-border px-3 py-2">
