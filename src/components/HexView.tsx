...
export function HexView({ chip }: { chip: MemoryChip }) {
  ...
  return (
    // A part bigger than a few KB would want virtualizing; the declared ones
    // are 256 bytes, so a plain scroll container is honest and cheap.
...
function useMemorySnapshot(chip: MemoryChip) {
  ...
  useEffect(() => {
    ...
    let previous = chip.memory.slice()
