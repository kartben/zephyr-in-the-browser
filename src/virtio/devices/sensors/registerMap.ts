/**
 * JSON register maps for declarative I²C sensors — SVD-inspired, not SVD.
 *
 ...
 * the bitfields a human wants to see. Channel codecs (engineering units → wire
 * words) stay in TypeScript because they are functions; the register file
 * itself can live as JSON next to the chip and be merged in at build time.
 *
 * Shape (see maps/*.json):
 *
 * ```json
 ...
 *       "reset": "0x60A0",
 *       "fields": [
 *         { "name": "EM", "lsb": 4, "msb": 4, "description": "Extended mode" }
 *       ]
 *     }
 *   ]
 * }
...
import type { Endian, FieldDecl, RegisterDecl } from './model'
...
/** Turn a JSON register map into {@link RegisterDecl}s the sensor machine uses. */
 ...
 * register list, matched by address. Useful when a chip already declares its
 * registers in TypeScript and a map only adds human metadata.
 */
export function mergeRegisterMap(
  ...
  return registers.map((reg) => {
    ...
    return {
      ...
      // Geometry stays with the live declaration — JSON metadata must not
      // silently change what the bus returns.
    }
  })
}
