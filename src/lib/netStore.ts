 ...
 * (default, needs nothing) or the WebSocket uplink to a self-hosted passt
 * gateway (docs/net-gateway.md). One record for the one NIC every board has
 ...
 * `?net=` query > stored settings > default (sim). An unparseable query
 * value is warned about and ignored rather than silently changing modes.
 */
