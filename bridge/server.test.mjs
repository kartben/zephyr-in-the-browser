/**
 * Bridge lifecycle tests. Net/passt integration can be exercised with a fake
 * passt binary via startPasstSessionFn hooks; when passt is not available on the
 * host, tests still pass with netReady false (enableNet defaults off in tests).
 */
...
async function withBridge(fn, overrides = {}, hookOverrides = {}) {
...
test('hello lists ports; select-serial streams CTF', async () => {
  await withBridge(async (bridge, getSerial) => {
    ...
    assert.equal(msg.protocol, 'zitb-bridge')
    assert.deepEqual(msg.features, { ctf: true, gdb: true, net: false })
...
test('healthz reports ok', async () => {
  await withBridge(async (bridge) => {
    ...
    assert.equal(body.features.net, false)
    assert.equal(body.net.ready, false)
  })
})

test('auto-serial opens the sole port', async () => {
  await withBridge(
    async (bridge, getSerial) => {
      const snap = bridge.snapshot()
      assert.equal(snap.serial.phase, 'streaming')
      assert.equal(snap.serial.path, '/dev/ttyONLY')
      const serial = getSerial()
      assert.ok(serial)
      assert.equal(serial.path, '/dev/ttyONLY')
    },
    { autoSerial: true },
    { listPortsFn: async () => fakePorts(['/dev/ttyONLY']) },
  )
})

test('startProbeBridge is an alias for startBridge', async () => {
  const { startProbeBridge } = await import('./server.mjs')
  assert.equal(startProbeBridge, startBridge)
})
