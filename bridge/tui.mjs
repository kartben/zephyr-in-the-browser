/**
 * Compact full-screen TUI for the bridge.
 *
...
/**
 * @param {{
 *   bridge: Awaited<ReturnType<import('./server.mjs').startBridge>>,
 *   wsUrl: string,
 *   deepLink: string,
 *   baudRate: number,
...
export async function runTui(opts) {
  ...
  const draw = () => {
    const lines = []
    lines.push(`${BOLD}Zephyr bridge${RESET}  ${DIM}v${snap.version}${RESET}`)
    lines.push('')
    ...
    const netReady = snap.features?.net ?? snap.net?.ready ?? false
    const netColor = netReady ? GREEN : DIM
    lines.push(
      `${BOLD}Net${RESET}   ${netColor}${netReady ? 'ready' : 'unavailable'}${RESET}` +
        (netReady ? '' : `  ${DIM}(install passt or unset DISABLE_NET)${RESET}`),
    )
    lines.push(
  ...
  bridge.subscribeStatus((snap) => {
    const ser =
      snap.serial.phase === 'streaming'
        ...
        : snap.serial.phase
    console.log(
      `[bridge] status clients=${snap.clients} serial=${ser} gdb=${snap.gdb.phase} net=${snap.features?.net ?? snap.net?.ready ?? false} ports=${snap.ports.length}`,
    )
  })
  await new Promise(() => {})
}
