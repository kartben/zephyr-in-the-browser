...
| DNS resolves but TCP hangs (`http_get` prints `Error: -1`, echo/HTTP servers stall) | The gateway's passt predates 2026_07_16: older releases silently drop IPv4 frames padded to the 60-byte Ethernet minimum, which is every short TCP segment (SYN, pure ACK, FIN) from guests whose NICs pad like real hardware — the Cortex-M3's does. UDP frames are long enough to pass, which is what makes it look like a DNS/app bug. Pull or rebuild the current image (it builds passt 2026_07_16 from source); the app also trims the padding client-side, so a current page works even against an old gateway. |
