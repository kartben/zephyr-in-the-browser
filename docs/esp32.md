...
and for context the other boards land like this on the same method: the
Cortex-M3 had already printed before the probe could attach at 5.3 s, the C3
finished before 8.4 s, and the ESP32 at shift 5 finished at 11.8 s and 14.9 s
against 35.2 s and 48.8 s at shift 3 (all page-load relative, so all including
the 5 to 8 s of asset fetch and wasm instantiation every board pays). The M3 and
A53 run no boot ROM at all, which is most of why they are instant; the C3 and
the ESP32 both boot the real Espressif ROM out of flash, and the ESP32's does
more work on a dual-core part.
Timed from the guest's own output rather than from a screenshot, with the shift
alternated run to run so the result tracks the shift and not elapsed time:

| run | `-icount` | host load | guest time to last byte |
| --- | --- | --- | --- |
| 1 | 5 | 3.3 | 7.4 s |
| 2 | 3 | 7.3 | 25.8 s |
| 3 | 5 | 5.6 | 4.3 s |
...
Runs 3 and 4 are the closest-matched pair on load: 4.3 s against 41.4 s. The gap
is wider in the browser than the 3.2x measured natively, not narrower.
