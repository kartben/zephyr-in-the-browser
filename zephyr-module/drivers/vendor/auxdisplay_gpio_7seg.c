 ...
 *
 * Based on Zephyr's drivers/auxdisplay/auxdisplay_gpio_7seg.c (a258a4b017e6).
 *
 * Upstream runs the multiplex refresh — including blocking gpio_pin_set_dt() —
 * directly from the k_timer expiry ISR. That is illegal for any GPIO backend
 * that cannot complete in interrupt context (notably virtio,gpio: every
 * SET_VALUE is a virtqueue round trip). Under qemu-wasm the console then
 * deadlocks: the QEMU worker blocks on the TTY SharedArrayBuffer while the
 * page drains an ISR-originated GPIO flood, leaving a blank terminal.
 *
 * This copy keeps the same DT compatible and auxdisplay API, but drives the
 * refresh from a delayed work item on the system workqueue so all GPIO calls
 * run in thread context. Retire it once upstream moves the refresh out of ISR.
 */
...
struct auxdisplay_gpio_7seg_data {
	...
	struct k_work_sync refresh_work_sync;
...
static void auxdisplay_gpio_7seg_blank(const struct auxdisplay_gpio_7seg_config *cfg)
{
...
static void auxdisplay_gpio_7seg_refresh(struct k_work *work)
{
	struct k_work_delayable *dwork = k_work_delayable_from_work(work);
	struct auxdisplay_gpio_7seg_data *data =
		CONTAINER_OF(dwork, struct auxdisplay_gpio_7seg_data, refresh_work);
	const struct device *dev = data->dev;
	const struct auxdisplay_gpio_7seg_config *cfg = dev->config;

	if (!data->active) {
		auxdisplay_gpio_7seg_blank(cfg);
		data->refresh_pos = 0;
		return;
	}
	...
	for (uint32_t i = 0; i < cfg->segment_count; i++) {
		gpio_pin_set_dt(&cfg->segment_gpios[i],
				(cfg->buffer[data->refresh_pos] & BIT(i)) != 0);
	}

	/* Turn on the new digit */
	gpio_pin_set_dt(&cfg->digit_gpios[data->refresh_pos], 1);

	(void)k_work_schedule(&data->refresh_work, K_MSEC(cfg->refresh_period_ms));
}
...
static int auxdisplay_gpio_7seg_display_on(const struct device *dev)
{
	...
	data->refresh_pos = 0;
	data->active = true;
	(void)k_work_schedule(&data->refresh_work, K_NO_WAIT);
...
static int auxdisplay_gpio_7seg_display_off(const struct device *dev)
{
	...
	data->active = false;
	(void)k_work_cancel_delayable_sync(&data->refresh_work, &data->refresh_work_sync);
	auxdisplay_gpio_7seg_blank(cfg);
	data->refresh_pos = 0;
...
static int auxdisplay_gpio_7seg_init(const struct device *dev)
{
	...
	k_work_init_delayable(&data->refresh_work, auxdisplay_gpio_7seg_refresh);

	return auxdisplay_gpio_7seg_display_on(dev);
}
