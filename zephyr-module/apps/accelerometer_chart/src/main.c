 ...
 *
 * Browser-tuned fork of Zephyr's samples/modules/lvgl/accelerometer_chart.
 *
 * The stock sample uses LV_CHART_UPDATE_MODE_SHIFT on a full-screen chart.
 * Under qemu-wasm's ~66 MIPS A53 that mode redraws every series on every
 * sample and falls behind wall-clock time — the display looks "stuck" even
 * when I²C is idle and the sensor values are barely moving. Circular update
 * keeps a live trace without shifting the whole plot, and the matching
 * accel-display snippet shrinks ramfb so each refresh has fewer pixels to
 * fill. Sampling rate / point count stay in Kconfig (see conf/lvgl-accel.conf).
 */
...
static void create_accelerometer_chart(lv_obj_t *parent)
{
	...
	/* Fewer grid lines than upstream (5×8): each is a full-width stroke. */
	lv_chart_set_div_line_count(chart1, 3, 4);
	lv_chart_set_range(chart1, LV_CHART_AXIS_PRIMARY_Y, -20, 20); /* roughly -/+ 2G */
	/*
	 * Circular (ECG-style) update: overwrite the oldest point in place
	 * instead of SHIFT's "move every point left". Same API, far less work
	 * per sample on a software-rendered framebuffer.
	 */
