 ...
 *
 * Adapted from upstream samples/drivers/can/counter: rx_thread and
 * poll_state_thread each carry a 512-byte stack sized for the small MCUs this
 * sample usually runs on. Over virtio-spi, can_get_state()/can_add_rx_filter()
 * route through the mcp2515 driver into spi_virtio_transceive() and Zephyr's
 * virtqueue core, and AArch64's wider registers roughly double the size of
 * every frame in that chain versus a 32-bit target. Measured against the
 * built ELF, the straight-line frame chain for a single poll_state_thread
 * register read already summed to 624 bytes before any leaf call — the
 * threads reliably overflowed and corrupted adjacent memory (a wild-pointer
 * data abort a few calls downstream, not a clean stack-guard fault). 2048
 * bytes each covers that with real headroom. See docs/can-bus.md for the
